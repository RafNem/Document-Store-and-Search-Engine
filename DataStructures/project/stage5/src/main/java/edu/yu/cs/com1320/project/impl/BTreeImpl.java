...
public class BTreeImpl<Key extends Comparable<Key>, Value> implements BTree<Key, Value> {
	...
	public Value get(Key k) {
		...
		if (entry != null) {
			if (entry.val != null) {
				val = (Value) entry.val;
			} else if (entry.val == null && this.diskKeys.remove(k)) {
				try {
					val = (Value)persistenceManager.deserialize(k);
					put(k, val);
				} catch (IOException e) {
	...
	public Value put(Key k, Value v) {
		...
		if (v == null) {
			Entry entry = get(this.root, (Key) k, this.height);
			Value val = null;
			if (entry != null) {
				if (entry.val != null) {
					val = (Value) entry.val;
		...
		if (entry != null) {
			if (entry.val != null) {
				...
				entry.val = v;
				return val;
			} else if (entry.val == null && this.diskKeys.contains(k)) {
				try {
					val = (Value) persistenceManager.deserialize(k);
					diskKeys.remove(k);
					entry.val = v;
	...
	public void moveToDisk(Key k) throws Exception {
		...
		persistenceManager.serialize(k, get(this.root, (Key) k, this.height).val);
		get(this.root, (Key) k, this.height).val = null;
		this.diskKeys.add((Key)k);
	}
